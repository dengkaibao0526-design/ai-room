...
export default function KBZeroGame() {
  ...
  useEffect(() => {
    ...
    function swapWeapon(index = (state.weapon + 1) % WEAPONS.length) {
      ...
      state.fireHeld = false; state.weaponKick = 0.7;
      audio.swap(); haptic(12);
    }
    function shoot() {
      const w = weapon();
      ...
      if (!best) { burst(innerWidth * 0.5, innerHeight * 0.5, 7, 0.35); return; }
    ...
    function fixedUpdate(dt) {
      ...
      const keyStrafe = (keys.has("KeyD") || keys.has("ArrowRight") ? 1 : 0) - (keys.has("KeyA") || keys.has("ArrowLeft") ? 1 : 0);
      ...
      if (state.reloadTimer > 0) {
        ...
        if (state.reloadTimer <= 0) {
          const w = weapon();
          const loaded = Math.min(w.mag - state.ammo[state.weapon], state.reserve[state.weapon]);
      ...
      for (let i = state.particles.length - 1; i >= 0; i -= 1) { const p = state.particles[i]; p.life -= dt; p.x += p.vx * dt; p.y += p.vy * dt; p.vx *= Math.exp(-3.5 * dt); p.vy *= Math.exp(-3.5 * dt); if (p.life <= 0) state.particles.splice(i, 1); }
      for (let i = state.hippos.length - 1; i >= 0; i -= 1) { const hippo = state.hippos[i]; hippo.life -= dt; hippo.x += hippo.vx * dt; hippo.y += hippo.vy * dt; hippo.vy += 620 * dt; hippo.rot += hippo.spin * dt; if (hippo.life <= 0) state.hippos.splice(i, 1); }
    }
    function draw() {
      ...
      const sky = ctx.createLinearGradient(0, 0, 0, horizon); sky.addColorStop(0, "#07050f"); sky.addColorStop(0.72, "#141020"); sky.addColorStop(1, "#2a1744"); ctx.fillStyle = sky; ctx.fillRect(0, 0, w, Math.max(0, horizon));
      const floor = ctx.createLinearGradient(0, horizon, 0, h); floor.addColorStop(0, "#181020"); floor.addColorStop(1, "#040406"); ctx.fillStyle = floor; ctx.fillRect(0, Math.max(0, horizon), w, h - horizon);
      const columns = Math.min(900, Math.max(320, Math.floor(w * 0.72)));
      const columnWidth = w / columns;
      for (let i = 0; i < columns; i += 1) {
        const sx = i * columnWidth;
        const angle = state.angle + (i / columns - 0.5) * fov;
      ...
      const visible = state.enemies.filter((enemy) => enemy.alive).map((enemy) => {
        ...
        return { enemy, distance: Math.hypot(dx, dy), delta: angleDelta(Math.atan2(dy, dx), state.angle) };
      ...
      const bobY = Math.sin(state.bob) * Math.min(7, moveSpeed * 1.35), bobX = Math.cos(state.bob * 0.5) * Math.min(5, moveSpeed * 0.8), ads = state.ads ? 1 : 0;
      ...
      const gunGradient = ctx.createLinearGradient(-120, -50, 120, 70); gunGradient.addColorStop(0, "#121018"); gunGradient.addColorStop(0.55, state.weapon ? "#1f2c4e" : "#33204d"); gunGradient.addColorStop(1, state.weapon ? "#6aa7ff" : "#8155ef");
      ctx.fillStyle = gunGradient; ctx.beginPath();
      ...
      ctx.closePath(); ctx.fill(); ctx.fillStyle = "rgba(226,211,255,.82)"; ctx.fillRect(18, -23, state.weapon ? 104 : 72, 4); ctx.fillStyle = "#07070b"; ctx.fillRect(state.weapon ? 116 : 102, -16, state.weapon ? 70 : 54, 15);
      if (state.muzzle > 0.05) { ctx.globalAlpha = clamp(state.muzzle, 0, 1); const fx = state.weapon ? 188 : 158; const flash = ctx.createRadialGradient(fx, -9, 0, fx, -9, 72); flash.addColorStop(0, "#fff"); flash.addColorStop(0.18, "#d9b4ff"); flash.addColorStop(1, "rgba(138,75,255,0)"); ctx.fillStyle = flash; ctx.beginPath(); ctx.arc(fx, -9, 72, 0, TWO_PI); ctx.fill(); }
      ctx.restore();
      const cx = w / 2, cy = h / 2, gap = 7 + (state.ads ? 4 : 0) + moveSpeed * 0.8;
      ctx.strokeStyle = state.hitFlash > 0.15 ? "rgba(255,255,255,.98)" : "rgba(221,201,255,.8)"; ctx.lineWidth = 1.4; ctx.beginPath();
      ctx.moveTo(cx-gap-9,cy); ctx.lineTo(cx-gap,cy); ctx.moveTo(cx+gap,cy); ctx.lineTo(cx+gap+9,cy); ctx.moveTo(cx,cy-gap-9); ctx.lineTo(cx,cy-gap); ctx.moveTo(cx,cy+gap); ctx.lineTo(cx,cy+gap+9); ctx.stroke();
      for (const p of state.particles) { const alpha = clamp(p.life / p.max, 0, 1); ctx.fillStyle = `rgba(190,145,255,${alpha})`; ctx.beginPath(); ctx.arc(p.x, p.y, p.size * alpha, 0, TWO_PI); ctx.fill(); }
      for (const hippo of state.hippos) { const t = clamp(hippo.life / hippo.max, 0, 1); const pop = Math.sin((1 - t) * Math.PI) * 0.42 + 0.72; ctx.save(); ctx.translate(hippo.x, hippo.y); ctx.rotate(hippo.rot); ctx.globalAlpha = Math.min(1, t * 2.4); ctx.font = `${Math.round(76 * hippo.scale * pop)}px Apple Color Emoji, Segoe UI Emoji, sans-serif`; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.shadowBlur = 28; ctx.shadowColor = "rgba(158,91,255,.9)"; ctx.fillText("🦛", 0, 0); ctx.restore(); }
    ...
    function onKeyUp(e) { if (CONTROL_CODES.has(e.code)) e.preventDefault(); keys.delete(e.code); }
    function onMouseMove(e) {
      if (document.pointerLockElement !== canvas) return;
      const sensitivity = (state.ads ? 0.00105 * state.sens.ads : 0.0017) * state.sens.mouse;
    ...
    function onMouseUp(e) { if (e.button === 0) state.fireHeld = false; if (e.button === 2) state.ads = false; }
    function onBlur() { state.fireHeld = false; state.ads = false; keys.clear(); state.mobileMove = { x: 0, y: 0 }; }
  ...
  function updateSens(key, value) {
    const next = { ...sens, [key]: Number(value) };
...
function SensitivityPanel({ sens, onChange }) {
  return <div className="zeroSensPanel">
    <strong>灵敏度</strong>
    <label>鼠标 <input type="range" min="0.35" max="2.2" step="0.05" value={sens.mouse} onChange={(e) => onChange("mouse", e.target.value)} /><em>{sens.mouse.toFixed(2)}</em></label>
    <label>移动端视角 <input type="range" min="0.35" max="2.6" step="0.05" value={sens.touch} onChange={(e) => onChange("touch", e.target.value)} /><em>{sens.touch.toFixed(2)}</em></label>
    <label>开镜 <input type="range" min="0.35" max="1.2" step="0.05" value={sens.ads} onChange={(e) => onChange("ads", e.target.value)} /><em>{sens.ads.toFixed(2)}</em></label>
  </div>;
}
...
function MobileControls({ onMove, onLook, onFire, onAds, onReload, onSwap }) {
  ...
  function moveStart(e) { stopEvent(e); const t = e.changedTouches[0]; moveState.current = { id: t.identifier, ox: t.clientX, oy: t.clientY }; }
  function moveTouch(e) { stopEvent(e); const s = moveState.current; const t = Array.from(e.changedTouches).find((x) => x.identifier === s.id); if (!t) return; onMove(clamp((t.clientX - s.ox) / 56, -1, 1), clamp((t.clientY - s.oy) / 56, -1, 1)); }
  function moveEnd(e) { stopEvent(e); onMove(0, 0); moveState.current = { id: null, ox: 0, oy: 0 }; }
  function lookStart(e) { stopEvent(e); const t = e.changedTouches[0]; lookState.current = { id: t.identifier, x: t.clientX, y: t.clientY }; }
  function lookTouch(e) { stopEvent(e); const s = lookState.current; const t = Array.from(e.changedTouches).find((x) => x.identifier === s.id); if (!t) return; onLook(t.clientX - s.x, t.clientY - s.y); s.x = t.clientX; s.y = t.clientY; }
  return <div className="zeroMobileControls">
    <div className="zeroMovePad" onTouchStart={moveStart} onTouchMove={moveTouch} onTouchEnd={moveEnd} onTouchCancel={moveEnd}><i /></div>
    <div className="zeroLookPad" onTouchStart={lookStart} onTouchMove={lookTouch} />
    <button className="zeroFire zeroFireLeft" type="button" onTouchStart={(e) => { stopEvent(e); onFire(true); }} onTouchEnd={(e) => { stopEvent(e); onFire(false); }}>FIRE</button>
    <button className="zeroFire zeroFireRight" type="button" onTouchStart={(e) => { stopEvent(e); onFire(true); }} onTouchEnd={(e) => { stopEvent(e); onFire(false); }}>FIRE</button>
    <button className="zeroAds" type="button" onTouchStart={(e) => { stopEvent(e); onAds(true); }} onTouchEnd={(e) => { stopEvent(e); onAds(false); }}>ADS</button>
